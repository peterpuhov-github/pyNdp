...
class TpchQ14:
    def __init__(self, file_name: str, row_group: int):
        ...
        self.projection_columns = ['l_partkey', 'l_extendedprice', 'l_discount']
        total_columns = filter_columns + self.projection_columns
        ...
        self.df = self.df[self.projection_columns]
        print(f'projected columns {self.df.columns}')

    def to_spark(self, outfile):
        header = numpy.empty(len(self.projection_columns) + 1, numpy.int64)
        dtypes = [DataTypes.type[self.df.dtypes[c].name] for c in self.df.columns]
        header[0] = len(self.projection_columns)
        i = 1
        for t in dtypes:
            header[i] = t
            i += 1
